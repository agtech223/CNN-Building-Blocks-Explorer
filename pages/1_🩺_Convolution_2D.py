...
def calculate_output_size(input_size, kernel_size, stride, padding, dilation=1):
    """
    Calculate the size of the output after a convolution operation.
    Formula:
...
def main():
    st.set_page_config(page_title="PyTorch Conv2D Explorer", layout="wide")
    st.title("🔍 Image Conv2D Explorer with PyTorch")
    st.write("""
        Upload an image, configure Conv2D parameters, choose the convolution type, apply the convolution, and visualize the resulting feature maps along with detailed convolution information.
    """)

    # Sidebar for Conv2D parameters
    st.sidebar.header("🔧 Conv2D Parameters")

    # Convolution Type Selection
    conv_type = st.sidebar.selectbox("🔄 Convolution Type", options=["Standard", "Depthwise"], index=0)

    # Kernel size input
    kernel_size = st.sidebar.slider("📐 Kernel Size", min_value=1, max_value=11, value=3, step=1)
    if kernel_size % 2 == 0:
        st.sidebar.warning("⚠️ Kernel size is even. For 'same' padding, it's recommended to use odd numbers.")
    
    kernel_size_tuple = (kernel_size, kernel_size)
    
    # Number of filters
    if conv_type == "Standard":
        num_filters = st.sidebar.slider("🎨 Number of Filters", min_value=1, max_value=64, value=8, step=1)
    else:
        num_filters_per_channel = st.sidebar.slider("🎨 Number of Filters per Channel", min_value=1, max_value=32, value=8, step=1)
        num_filters = num_filters_per_channel  # Will be used differently for depthwise
    ...
    if uploaded_file is not None:
        try:
            ...
            mode = st.selectbox("🎨 Select Image Mode for Convolution", options=["RGB", "Grayscale"], index=0)
            ...
            # Button to apply Conv2D
            if st.button("🚀 Apply Conv2D"):
                with st.spinner('🔄 Applying Conv2D...'):
                    # Define Conv2D layer based on convolution type
                    in_channels = image_tensor.shape[1]
                    if conv_type == "Standard":
                        out_channels = num_filters
                        groups = 1
                    else:
                        # Depthwise: groups = in_channels, out_channels = in_channels * num_filters_per_channel
                        groups = in_channels
                    ...
                    # Display convolution parameters and output tensor shape
                    st.markdown("### 📊 Convolution Details")
                    if conv_type == "Standard":
                        st.markdown(f"- **Filter Size (Kernel):** {kernel_size_tuple}")
                        st.markdown(f"- **Number of Filters (Output Channels):** {num_filters}")
                    else:
                        st.markdown(f"- **Filter Size (Kernel):** {kernel_size_tuple} per channel")
                        st.markdown(f"- **Number of Filters per Channel:** {num_filters}")
                        st.markdown(f"- **Total Number of Filters (Output Channels):** {out_channels}")
                    ...
                    max_cols = 4
                    num_filters_to_display = out_channels
                    ...
                    st.success("✅ Conv2D applied successfully!")
